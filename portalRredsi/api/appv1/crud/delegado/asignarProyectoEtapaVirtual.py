...
def asignar_proyecto_etapa_virtual(db: Session, asignacion: AsignarProyectoEtapaUno ):
    try:
        sql = text("INSERT INTO participantes_proyecto (id_datos_personales, id_proyecto, id_etapa, id_proyecto_convocatoria, tipo_participante) VALUES (:id_dp, :id_p, :id_etp, :id_pc, :tipo_p)")
...
def get_convocatoria_actual_por_proyecto(db: Session, id_proyecto: int):
    sql = text("SELECT * FROM proyectos_convocatoria JOIN convocatorias ON proyectos_convocatoria.id_convocatoria = convocatorias.id_convocatoria  WHERE proyectos_convocatoria.id_proyecto = :id_p AND convocatorias.estado = 'en curso'")
