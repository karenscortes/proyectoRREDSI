<template>
    <div class="container">
              ...
              <form @submit.prevent="handleSubmit('personal')" class="mt-4 font-weight-bold text-dark">
                ...
                <div class="form-row">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" v-model="formData.personal.numeroDocumento" class="form-control" id="inputCity" />
                  </div>
                </div>
                <div class="form-group mb-5">
                  <label for="inputAddress">Correo:</label>
                  <input type="email" v-model="formData.personal.correo" class="form-control" id="inputAddress" placeholder="usuario@gmail.com" />
                </div>
              ...
              <form @submit.prevent="handleSubmit('institucional')" class="mt-4 font-weight-bold text-dark">
                <div class="form-row">
                  ...
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" v-model="formData.institucional.grupoInvestigacion" class="form-control" id="inputSearchGroup" />
                  </div>
                </div>
                <div class="form-group mb-5">
              ...
              <form @submit.prevent="handleSubmit('academico')" class="mt-4 font-weight-bold text-dark">
                ...
                <div class="form-row">
                  <div class="form-group col-md-4">
                    ...
                    <div class="d-inline-flex">
                      <input type="text" v-model="formData.academico.doctorado" class="form-control" id="inputPhD" />
                      <label class="items-center p-1 text-black">
                        <i class="fas fa-plus-circle h4"></i>
                        <input type="file" class="d-none d-print-block" />
                      </label>
                  ...
                  <div class="form-group col-md-4 pr-5">
                    <label for="inputKnowledgeArea">Área de Conocimiento:</label>
                    <select v-model="formData.academico.areaConocimiento" id="inputKnowledgeArea" class="form-control text-dark custom-select" required>
                      <option value="" disabled selected>Seleccione una opción</option>
                      <option value="IA">Inteligencia Artificial</option>
                    </select>
                  </div>
                  <div class="form-group col-md-4 pr-5 mb-5">
                    <label for="inputAnotherArea">Otra Área:</label>
                    <select v-model="formData.academico.otraArea" id="inputAnotherArea" class="form-control text-dark custom-select" required>
                      <option value="" disabled selected>Seleccione una opción</option>
      ...
      <div class="text-center mt-4">
        <button @click="saveProfile" class="btn btn-primary">Guardar Cambios</button>
      </div>
  
      <!-- Modal para cambiar contraseña -->
      <modal-change-password v-if="isChangePasswordModalVisible" @close="isChangePasswordModalVisible = false" />
    </div>
  </template>
  
  <script>
  import { updateUserProfile } from '../../services/UsuarioService'; // Suponiendo que existe la función getUserProfile
  ...
  export default {
    ...
    methods: {
      ...
      async loadUserProfile() {
        try {
          const userId = 'ID_DE_USUARIO'; // Reemplaza con la lógica para obtener el ID del usuario
      ...
      handleSubmit(section) {
        console.log(`Submitting ${section} data`, this.formData[section]);
      },
      ...
      async saveProfile() {
        try {
          const userId = 'ID_DE_USUARIO'; // Reemplaza con la lógica para obtener el ID del usuario
          await updateUserProfile(userId, this.formData);
    ...
    mounted() {
      this.loadUserProfile(); // Cargar el perfil cuando se monta el componente
    },
  };
  </script>
