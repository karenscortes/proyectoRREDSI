        ...
        <form class="mt-4" @submit.prevent="guardarDatosProyecto">
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <input v-model="datosProyecto.institucion_educativa" type="text" class="form-control"
                id="institucion_educativa" placeholder="Nombre de la institución" required />
            </div>
            <div class="col-md-6">
              ...
              <input v-model="datosProyecto.programa_academico" type="text" class="form-control"
                id="programa_academico" placeholder="Nombre del programa" required />
            </div>
          </div>
          ...
          <div class="row mb-3">
            <div class="col-md-12">
              ...
              <input type="file" @change="handleFileUpload($event, 'aval')" class="form-control" id="aval" required />
            </div>
          </div>
        ...
        <form class="mt-4" @submit.prevent="guardarDatosPonente">
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <input v-model="datosPonente.nombres" type="text" class="form-control" id="nombres_ponente" required />
            </div>
            <div class="col-md-6">
              <label for="apellidos_ponente" class="form-label text-black">Apellidos:</label>
              <input v-model="datosPonente.apellidos" type="text" class="form-control" id="apellidos_ponente" required />
            </div>
          </div>
          <div class="row mb-3">
            <div class="col-md-6">
              <label for="celular_ponente" class="form-label text-black">Celular:</label>
              <input v-model="datosPonente.celular" type="text" class="form-control" id="celular_ponente" required />
            </div>
            <div class="col-md-6">
              <label for="correo_ponente" class="form-label text-black">Correo Electrónico:</label>
              <input v-model="datosPonente.correo" type="email" class="form-control" id="correo_ponente" required />
            </div>
          </div>
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title">Proyecto Registrado</h5>
              <button type="button" class="close" @click="cerrarModal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div class="modal-body">
              <p>El proyecto ha sido registrado con éxito. El código de su proyecto es {{ proyectoCodigo }}.</p>
            </div>
    ...
    setup() {
      ...
      const registrarProyecto = async () => {
        try {
          ...
          formData.append('id_institucion', datosProyecto.value.institucion_educativa);
          formData.append('programa_academico', datosProyecto.value.programa_academico);
          formData.append('modalidad', datosProyecto.value.modalidad);
          formData.append('titulo', datosProyecto.value.titulo);
          formData.append('propuesta_escrita', datosProyecto.value.propuesta_escrita);
          formData.append('area_conocimiento', datosProyecto.value.area_conocimiento);
          formData.append('aval', datosProyecto.value.aval);
  
          const autoresNombres = autores.value.map((autor) => autor.nombre);
          formData.append('autores', autoresNombres);
  
          const response = await axios.post('/projects/projects/create-project', formData);
